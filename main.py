...
import config
import database # Bu sətir indi düzgün yerdədir və xətanı həll etməlidir!
...
def get_main_menu_keyboard():
    keyboard = [
        [InlineKeyboardButton("Balans artır", callback_data="add_balance")],
        [InlineKeyboardButton("Balansa baxmaq", callback_data="check_balance")],
        [InlineKeyboardButton("Xidmətlər", callback_data="services_menu")],
        [InlineKeyboardButton("Adminlə əlaqə", callback_data="contact_admin")],
    ]
...
def is_valid_url(url: str) -> bool:
    ...
    regex = re.compile(
        r'^(https?://)?(www\.)?(telegram\.me/|t\.me/|instagram\.com/|tiktok\.com/)' # Əlavə domenlər əlavə edə bilərsiniz
        r'[a-zA-Z0-9.\-/?&=#_]*$'
    )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    database.init_db() # Veritabanını başlatmaq (hər startda lazım deyil, amma əmin olmaq üçün qoyulub)
    await update.message.reply_text(
        "Salam! Aşağıdakı menyudan seçim edin:", reply_markup=get_main_menu_keyboard()
    )
...
""",
        ...
        # State already set above, just confirming

    elif query.data == "check_balance":
        balance = database.get_user_balance(user_id)
        await query.edit_message_text(f"Sizin cari balansınız: **{balance:.2f} AZN**.", parse_mode="Markdown")
...
Sifariş vermək üçün istədiyiniz sayı və xidməti qeyd edin (məs: "1k abuneci" və ya "10k baxış").""".format(
        ...
        USER_STATES[user_id] = AWAITING_ADMIN_REPLY # Adminin cavab vəziyyətinə keçir
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id == config.ADMIN_ID and USER_STATES.get(user_id) == AWAITING_ADMIN_PRICE_CHANGE_AMOUNT:
        ...
        # Admin mesaj ID-sini və istifadəçi ID-sini yadda saxla
        ...
        USER_STATES.pop(user_id, None) # Mesajı göndərdi, artıq gözləmirik

    else:
        # Xidmət sifarişlərini emal et
...
async def handle_photo_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if USER_STATES.get(user_id) == AWAITING_RECEIPT:
        ...
        USER_STATES.pop(user_id, None) # Çeki gönderdi, artık beklemiyoruz
    else:
...
async def handle_admin_set_price_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not query.data.startswith("set_price_"):
        return # Not a price change callback
